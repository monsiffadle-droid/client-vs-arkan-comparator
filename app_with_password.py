...
def check_password():
    ...
    def password_entered():
        ...
        username_ok = hmac.compare_digest(str(entered_username), str(correct_username))
        password_ok = hmac.compare_digest(str(entered_password), str(correct_password))
    ...
    .auth-wrap {
      ...
      background: white; border-radius: 18px; box-shadow: 0 10px 25px rgba(0,0,0,.08);
      border: 1px solid #e5e7eb;
    }
    .auth-title { font-size: 28px; font-weight: 700; margin-bottom: 8px; }
    .auth-sub { color: #475569; margin-bottom: 18px; }
    </style>
    <div class="auth-wrap">
      <div class="auth-title">🔐 Login Required</div>
      <div class="auth-sub">Please enter your username and password to access the tool.</div>
    </div>
